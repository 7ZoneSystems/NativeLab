...
class SmartReference:
    def __init__(self, ref_id: str, name: str, ftype: str, raw_text: str):
        ...
        self._disk_path = _cfg().REF_CACHE_DIR / f"{ref_id}.pkl"
        self._indexed   = False
    ...
    def get_chunk_text(self, idx: int) -> str:
        ...
        if ram_free_mb() > _cfg().RAM_WATCHDOG_MB + 200:
            self._hot[idx] = text
        return text
...
class ScriptSmartReference(SmartReference):
    def __init__(self, ref_id: str, name: str, lang_key: str, raw: str, cache_dir: Path):
        self.ref_id     = ref_id
        self.name       = name
        self.ftype      = "script"
...
class SessionReferenceStore:
    def __init__(self, session_id: str):
        ...
        self._index_path = _cfg().REF_INDEX_DIR / f"{session_id}_refs.json"
        self._load_meta()
    ...
    def _load_meta(self):
        ...
        try:
            ...
            for r in meta.get("refs", []):
                raw_path = _cfg().REF_CACHE_DIR / f"{r['ref_id']}_raw.txt"
    ...
    def save_meta(self):
        ...
        self._index_path.write_text(json.dumps({"refs": entries}), encoding="utf-8")
    ...
    def add_script_reference(self, name: str, raw: str) -> ScriptSmartReference:
        ...
        ref = ScriptSmartReference(ref_id, name, lang_key, raw, _cfg().REF_CACHE_DIR)
        self._refs[ref_id] = ref
        self.save_meta()
        return ref
