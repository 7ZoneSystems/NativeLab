...
{p(f'<b>How to add:</b><br>'
   f'&nbsp;&nbsp;&bull; <b>Drag</b> a model from the sidebar list onto the canvas &#8212; a ghost block shows the drop position.<br>'
   f'&nbsp;&nbsp;&bull; <b>Double-click</b> a model in the sidebar list to add it at a default position.')}
{p(f'<b>Right-click options:</b><br>'
   f'&nbsp;&nbsp;&bull; <b>Change Role</b> &#8212; sets the system prompt used for this block:<br>'
   f'&nbsp;&nbsp;&nbsp;&nbsp;{c_general} You are a helpful assistant.<br>'
   f'&nbsp;&nbsp;&nbsp;&nbsp;{c_reasoning} Think step by step.<br>'
   f'&nbsp;&nbsp;&nbsp;&nbsp;{c_summarization} Be clear and concise.<br>'
   f'&nbsp;&nbsp;&nbsp;&nbsp;{c_coding} Write clean, well-commented code.<br>'
   f'&nbsp;&nbsp;&bull; <b>Rename block</b> &#8212; give it a human-readable name.<br>'
   f'&nbsp;&nbsp;&bull; <b>Delete</b> &#8212; removes the block and all its connections.')}
{port_table(
...
{p(f'<b>Right-click &#8594; Configure block&#8230;</b> then choose:<br>'
   f'&nbsp;&nbsp;&bull; <b>Type / paste text</b> &#8212; opens a multiline input dialog.<br>'
   f'&nbsp;&nbsp;&bull; <b>Load from file</b> &#8212; reads any .txt, .md, .py, .json, .yaml file. '
...
{note(f'PyPDF2 must be installed: {c_pypdf2}. If it is missing the block will show a warning when you try to configure it.')}

{h2("&#9410;  Logic Blocks  ( Python conditions )")}
{p(f'Logic blocks evaluate Python expressions or perform text operations at runtime. '
   f'The variable {c_text_var} always holds the incoming context string. '
...
{example_box("CONDITION EXAMPLES",
    p(f'{c_len500} &#8212; route long responses to a summariser<br>'
      ...
      f'{c_startscode} &#8212; detect code output<br>'
      f'{c_shortwords} &#8212; detect very short answers<br>'
      f'{c_yescheck} &#8212; check if model said yes'))}
...
{example_box("EXPRESSION EXAMPLES",
    p(f'{c_longshort} &#8212; length-based routing<br>'
      ...
      f'{c_firstword} &#8212; route on first word / prefix<br>'
      f'{c_punctheur} &#8212; punctuation heuristic'))}
...
{p(f'<b>Available transforms:</b><br>'
   f'&nbsp;&nbsp;&bull; {c_prefix_t} &#8212; prepend fixed text before the context<br>'
   f'&nbsp;&nbsp;&bull; {c_suffix_t} &#8212; append fixed text after the context<br>'
   f'&nbsp;&nbsp;&bull; {c_replace_t} &#8212; find a substring and replace it<br>'
   f'&nbsp;&nbsp;&bull; {c_upper_t} / {c_lower_t} &#8212; change case<br>'
   f'&nbsp;&nbsp;&bull; {c_strip_t} &#8212; remove leading/trailing whitespace and blank lines<br>'
   f'&nbsp;&nbsp;&bull; {c_truncate_t} &#8212; cut text to a maximum number of characters')}
{tip(f'Chain multiple TRANSFORM blocks together to build a text preprocessing pipeline before it hits a model &#8212; e.g. {c_strip_t} &#8594; {c_truncate_t} &#8594; {c_prefix_t}.')}
...
{p(f'<b>Merge modes:</b><br>'
   f'&nbsp;&nbsp;&bull; {c_concat} &#8212; join with a separator string (default: two newlines + ---)<br>'
   f'&nbsp;&nbsp;&bull; {c_prepend_m} &#8212; newest first, oldest last<br>'
   f'&nbsp;&nbsp;&bull; {c_append_m} &#8212; oldest first, newest last<br>'
   f'&nbsp;&nbsp;&bull; {c_json_m} &#8212; wrap all inputs as a JSON array string')}
...
{p(f'<b>Available variables:</b><br>'
   f'&nbsp;&nbsp;&bull; {c_text_ro} &#8212; incoming context string (read-only)<br>'
   f'&nbsp;&nbsp;&bull; {c_result_var} &#8212; set this to your output string (defaults to {c_text_var} if not set)<br>'
   f'&nbsp;&nbsp;&bull; {c_metadata_var} &#8212; block metadata dict, persists across pipeline runs<br>'
   f'&nbsp;&nbsp;&bull; {c_log_fn} &#8212; writes a message to the Pipeline Log tab')}
{p(f'<b>Safe builtins available:</b> {c_safe_builtins}')}
...
{example_box("CODE EXAMPLES",
    p(f'{c_upper} &#8212; uppercase<br>'
      f'{c_sortlines} &#8212; sort lines<br>'
      f'{c_wordcount} &#8212; prepend word count<br>'
      f'{c_stateful} &#8212; stateful counter'))}
...
{h2("&#9889;  Performance Tips")}
{p(f'&bull; Keep context short between models &#8212; truncate with a TRANSFORM block before expensive model calls.<br>'
   f'&bull; Use small models (0.5B&#8211;1.5B) for all LLM logic routing blocks. Reserve big models for the actual generation steps.<br>'
   f'&bull; PDF blocks auto-summarise large documents &#8212; but this adds extra model calls. Pre-summarise large PDFs offline if speed matters.<br>'
   f'&bull; Loop iterations multiply model calls: 3 models &times; 5 loops = 15 server requests. Plan accordingly.<br>'
   f'&bull; Set Max response tokens conservatively on routing blocks (16 is enough for YES/NO decisions).')}
